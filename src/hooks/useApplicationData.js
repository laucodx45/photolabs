...
const useApplicationData = () => {
  ...
  useEffect(() => {
    const getPhotos = axios.get(`${serverUrl}/api/photos`);
    const getTopics = axios.get(`${serverUrl}/api/topics`);
  ...
  useEffect(() => {
    ...
    if (state.topicId !== null) {
      axios.get(`${serverUrl}/api/topics/photos/${state.topicId}`)
      ...
      axios.get(`${serverUrl}/api/photos`)
      .then(res => dispatch({ type: 'SET_PHOTO_DATA', payload: res.data }))
      .catch(err => console.log(`Error: ${err}`))
    }
  }, [state.topicId])
